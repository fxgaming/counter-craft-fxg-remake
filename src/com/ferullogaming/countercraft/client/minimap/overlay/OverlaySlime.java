      ...
      if (Minecraft.getMinecraft().thePlayer.getEntityWorld().provider.dimensionId != dim) {
         ...
         for(int x = limitMinX; x <= limitMaxX; ++x) {
            for(int z = limitMinZ; z <= limitMaxZ; ++z) {
               Random rnd = new Random(Minecraft.getMinecraft().theWorld.getSeed() + (long)(x * x * 4987142) + (long)(x * 5947611) + (long)(z * z) * 4392871L + (long)(z * 389711) ^ 987234911L);
               if (rnd.nextInt(10) == 0) {
                  chunks.add(new OverlaySlime.ChunkOverlay(x, z));
               }
            }
         }
