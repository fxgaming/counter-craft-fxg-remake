...
public class RegionFile {
   ...
   private RegionFile.Section[] chunkSectionsArray = new RegionFile.Section[4096];
   private int[] timestampArray = new int[4096];
   private List filledSectorArray = null;
   ...
   private void setFilledSectorArray(RegionFile.Section section, boolean filled) {
   ...
   private boolean checkSectionOverlaps(RegionFile.Section section) {
   ...
   private RegionFile.Section getFreeSection(int requiredLength) {
      int start = 0;
      int length = 0;
      int closestStart = 0;
      ...
      return new RegionFile.Section(closestStart, requiredLength);
   }
   ...
   public void printInfo() {
      ...
      for(i = 0; i < this.filledSectorArray.size(); ++i) {
         ...
         s = s + (((Boolean)this.filledSectorArray.get(i)).booleanValue() ? '1' : '0');
         if ((i & 31) == 31) {
            ;
         }
      }
   ...
   private RegionFile.Section getChunkSection(int x, int z) {
      return this.chunkSectionsArray[(z & 31) << 5 | x & 31];
   }

   private void updateChunkSection(int x, int z, RegionFile.Section newSection) throws IOException {
   ...
   public boolean open() {
      ...
      try {
         ...
         if (this.lengthInSectors < 3) {
            ...
            for(i = 0; i < 1024; ++i) {
               RegionFile.Section section = new RegionFile.Section(this.fin.readInt());
               if (section.length > 0 && !this.checkSectionOverlaps(section)) {
   ...
   public DataInputStream getChunkDataInputStream(int x, int z) {
      ...
      if (this.fin != null) {
         RegionFile.Section section = this.getChunkSection(x, z);
   ...
   public DataOutputStream getChunkDataOutputStream(int x, int z) {
      return new DataOutputStream(new DeflaterOutputStream(new RegionFile.RegionFileChunkBuffer(this, x, z)));
   }

   private void writeChunkDataToSection(RegionFile.Section section, byte[] compressedChunkData, int length) throws IOException {
   ...
   private boolean writeCompressedChunk(int x, int z, byte[] compressedChunkData, int length) {
      if (length <= 0) {
         ...
         RegionFile.Section currentSection = this.getChunkSection(x, z);
         if (currentSection != null) {
            this.setFilledSectorArray(currentSection, false);
         }

         int requiredSectors = (length + 5 + 4095) / 4096;
         RegionFile.Section newSection;
         if (currentSection != null && requiredSectors <= currentSection.length) {
            newSection = new RegionFile.Section(currentSection.startSector, requiredSectors);
         } else {
