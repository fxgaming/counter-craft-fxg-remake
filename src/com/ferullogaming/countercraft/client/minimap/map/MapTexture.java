...
import com.ferullogaming.countercraft.client.minimap.BackgroundExecutor;
import com.ferullogaming.countercraft.client.minimap.Texture;
import com.ferullogaming.countercraft.client.minimap.region.Region;
...
public class MapTexture extends Texture {
   ...
   private List textureUpdateQueue = new ArrayList();
   ...
   public void processTextureUpdates() {
      List var1 = this.textureUpdateQueue;
      synchronized(this.textureUpdateQueue) {
         Iterator i$ = this.textureUpdateQueue.iterator();

         while(i$.hasNext()) {
            MapTexture.Rect rect = (MapTexture.Rect)i$.next();
   ...
   public synchronized void setRGBOpaque(int x, int y, int w, int h, int[] pixels, int offset, int scanSize) {
      int bufOffset = y * super.w + x;

      for(int i = 0; i < h; ++i) {
         this.setPixelBufPosition(bufOffset + i * super.w);
   ...
   public void addTextureUpdate(int x, int z, int w, int h) {
      List var5 = this.textureUpdateQueue;
      synchronized(this.textureUpdateQueue) {
         this.textureUpdateQueue.add(new MapTexture.Rect(x, z, w, h));
      }
   }

   public void updateTextureFromRegion(Region region, int x, int z, int w, int h) {
      int tx = x >> region.zoomLevel & super.w - 1;
      int ty = z >> region.zoomLevel & super.h - 1;
      int tw = w >> region.zoomLevel;
      int th = h >> region.zoomLevel;
      tw = Math.min(tw, super.w - tx);
      th = Math.min(th, super.h - th);
