...
public class Texture {
   ...
   public Texture(int w, int h, int fillColour, int minFilter, int maxFilter, int textureWrap) {
      ...
      this.w = w;
      this.h = h;
      this.pixelBuf = MinimapUtils.allocateDirectIntBuffer(w * h);
   ...
   public Texture(int id) {
      ...
      this.pixelBuf = MinimapUtils.allocateDirectIntBuffer(this.w * this.h);
      this.getPixelsFromExistingTexture();
      MinimapUtils.log("created new MwTexture from GL texture id %d (%dx%d) (%d pixels)", this.id, this.w, this.h, this.pixelBuf.limit());
   }
   ...
   public synchronized void close() {
      if (this.id != 0) {
         try {
            ...
            MinimapUtils.log("MwTexture.close: null pointer exception (texture %d)", this.id);
         }
   ...
   public synchronized void updateTextureArea(int x, int y, int w, int h) {
      try {
         ...
         MinimapUtils.log("MwTexture.updatePixels: null pointer exception (texture %d)", this.id);
      }
   ...
   private synchronized void getPixelsFromExistingTexture() {
      try {
         ...
         MinimapUtils.log("MwTexture.getPixels: null pointer exception (texture %d)", this.id);
      }
