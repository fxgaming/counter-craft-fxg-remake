package com.ferullogaming.countercraft.client.model.hats;

import net.minecraft.client.model.ModelBase;
import net.minecraft.client.model.ModelRenderer;
import net.minecraft.entity.Entity;

public class ModelHatSkull extends ModelBase {
   ModelRenderer Shape2;
   ModelRenderer Shape3;
   ModelRenderer Shape4;
   ModelRenderer Shape5;
   ModelRenderer Shape6;
   ModelRenderer Shape7;
   ModelRenderer Shape8;
   ModelRenderer Shape9;
   ModelRenderer Shape10;
   ModelRenderer Shape11;
   ModelRenderer Shape12;
   ModelRenderer Shape13;
   ModelRenderer Shape14;
   ModelRenderer Shape15;
   ModelRenderer Shape16;
   ModelRenderer Shape17;
   ModelRenderer Shape18;
   ModelRenderer Shape19;
   ModelRenderer Shape20;
   ModelRenderer Shape21;
   ModelRenderer Shape22;
   ModelRenderer Shape23;
   ModelRenderer Shape24;
   ModelRenderer Shape25;
   ModelRenderer Shape26;
   ModelRenderer Shape27;
   ModelRenderer Shape28;
   ModelRenderer Shape29;
   ModelRenderer Shape30;

   public ModelHatSkull() {
      super.textureWidth = 64;
      super.textureHeight = 32;
      this.Shape2 = new ModelRenderer(this, 0, 28);
      this.Shape2.addBox(0.0F, 0.0F, 0.0F, 8, 3, 1);
      this.Shape2.setRotationPoint(0.0F, 0.0F, -1.0F);
      this.Shape2.setTextureSize(64, 32);
      this.Shape2.mirror = true;
      this.setRotation(this.Shape2, 0.0F, 0.0F, 0.0F);
      this.Shape3 = new ModelRenderer(this, 19, 29);
      this.Shape3.addBox(0.0F, 0.0F, 0.0F, 1, 2, 1);
      this.Shape3.setRotationPoint(0.0F, 3.0F, -1.0F);
      this.Shape3.setTextureSize(64, 32);
      this.Shape3.mirror = true;
      this.setRotation(this.Shape3, 0.0F, 0.0F, 0.0F);
      this.Shape4 = new ModelRenderer(this, 0, 24);
      this.Shape4.addBox(0.0F, 0.0F, 0.0F, 2, 2, 1);
      this.Shape4.setRotationPoint(3.0F, 3.0F, -1.0F);
      this.Shape4.setTextureSize(64, 32);
      this.Shape4.mirror = true;
      this.setRotation(this.Shape4, 0.0F, 0.0F, 0.0F);
      this.Shape5 = new ModelRenderer(this, 7, 24);
      this.Shape5.addBox(0.0F, 0.0F, 0.0F, 1, 2, 1);
      this.Shape5.setRotationPoint(7.0F, 3.0F, -1.0F);
      this.Shape5.setTextureSize(64, 32);
      this.Shape5.mirror = true;
      this.setRotation(this.Shape5, 0.0F, 0.0F, 0.0F);
      this.Shape6 = new ModelRenderer(this, 24, 30);
      this.Shape6.addBox(0.0F, 0.0F, 0.0F, 3, 1, 1);
      this.Shape6.setRotationPoint(0.0F, 5.0F, -1.0F);
      this.Shape6.setTextureSize(64, 32);
      this.Shape6.mirror = true;
      this.setRotation(this.Shape6, 0.0F, 0.0F, 0.0F);
      this.Shape7 = new ModelRenderer(this, 33, 30);
      this.Shape7.addBox(0.0F, 0.0F, 0.0F, 3, 1, 1);
      this.Shape7.setRotationPoint(5.0F, 5.0F, -1.0F);
      this.Shape7.setTextureSize(64, 32);
      this.Shape7.mirror = true;
      this.setRotation(this.Shape7, 0.0F, 0.0F, 0.0F);
      this.Shape8 = new ModelRenderer(this, 42, 30);
      this.Shape8.addBox(0.0F, 0.0F, 0.0F, 5, 1, 1);
      this.Shape8.setRotationPoint(1.5F, 6.0F, -1.0F);
      this.Shape8.setTextureSize(64, 32);
      this.Shape8.mirror = true;
      this.setRotation(this.Shape8, 0.0F, 0.0F, 0.0F);
      this.Shape9 = new ModelRenderer(this, 55, 30);
      this.Shape9.addBox(0.0F, 0.0F, 0.0F, 1, 1, 1);
      this.Shape9.setRotationPoint(1.5F, 7.0F, -1.0F);
      this.Shape9.setTextureSize(64, 32);
      this.Shape9.mirror = true;
      this.setRotation(this.Shape9, 0.0F, 0.0F, 0.0F);
      this.Shape10 = new ModelRenderer(this, 60, 30);
      this.Shape10.addBox(0.0F, 0.0F, 0.0F, 1, 1, 1);
      this.Shape10.setRotationPoint(3.5F, 7.0F, -1.0F);
      this.Shape10.setTextureSize(64, 32);
      this.Shape10.mirror = true;
      this.setRotation(this.Shape10, 0.0F, 0.0F, 0.0F);
      this.Shape11 = new ModelRenderer(this, 24, 27);
      this.Shape11.addBox(0.0F, 0.0F, 0.0F, 1, 1, 1);
      this.Shape11.setRotationPoint(5.5F, 7.0F, -1.0F);
      this.Shape11.setTextureSize(64, 32);
      this.Shape11.mirror = true;
      this.setRotation(this.Shape11, 0.0F, 0.0F, 0.0F);
      this.Shape12 = new ModelRenderer(this, 46, 18);
      this.Shape12.addBox(0.0F, 0.0F, 0.0F, 1, 3, 8);
      this.Shape12.setRotationPoint(-1.0F, 0.0F, 0.0F);
      this.Shape12.setTextureSize(64, 32);
      this.Shape12.mirror = true;
      this.setRotation(this.Shape12, 0.0F, 0.0F, 0.0F);
      this.Shape13 = new ModelRenderer(this, 0, 19);
      this.Shape13.addBox(0.0F, 0.0F, 0.0F, 1, 3, 1);
      this.Shape13.setRotationPoint(-1.0F, 3.0F, 0.0F);
      this.Shape13.setTextureSize(64, 32);
      this.Shape13.mirror = true;
      this.setRotation(this.Shape13, 0.0F, 0.0F, 0.0F);
      this.Shape14 = new ModelRenderer(this, 33, 22);
      this.Shape14.addBox(0.0F, 0.0F, 0.0F, 1, 2, 5);
      this.Shape14.setRotationPoint(-1.0F, 3.0F, 3.0F);
      this.Shape14.setTextureSize(64, 32);
      this.Shape14.mirror = true;
      this.setRotation(this.Shape14, 0.0F, 0.0F, 0.0F);
      this.Shape15 = new ModelRenderer(this, 48, 9);
      this.Shape15.addBox(0.0F, 0.0F, 0.0F, 1, 1, 7);
      this.Shape15.setRotationPoint(-1.0F, 5.0F, 1.0F);
      this.Shape15.setTextureSize(64, 32);
      this.Shape15.mirror = true;
      this.setRotation(this.Shape15, 0.0F, 0.0F, 0.0F);
      this.Shape16 = new ModelRenderer(this, 12, 24);
      this.Shape16.addBox(0.0F, 0.0F, 0.0F, 1, 2, 1);
      this.Shape16.setRotationPoint(-0.7F, 3.0F, 2.0F);
      this.Shape16.setTextureSize(64, 32);
      this.Shape16.mirror = true;
      this.setRotation(this.Shape16, 0.0F, 0.0F, 0.0F);
      this.Shape17 = new ModelRenderer(this, 19, 26);
      this.Shape17.addBox(0.0F, 0.0F, 0.0F, 1, 1, 1);
      this.Shape17.setRotationPoint(-0.7F, 3.0F, 1.0F);
      this.Shape17.setTextureSize(64, 32);
      this.Shape17.mirror = true;
      this.setRotation(this.Shape17, 0.0F, 0.0F, 0.0F);
      this.Shape18 = new ModelRenderer(this, 24, 24);
      this.Shape18.addBox(0.0F, 0.0F, 0.0F, 1, 1, 1);
      this.Shape18.setRotationPoint(-0.5F, 4.0F, 1.0F);
      this.Shape18.setTextureSize(64, 32);
      this.Shape18.mirror = true;
      this.setRotation(this.Shape18, 0.0F, 0.0F, 0.0F);
      this.Shape19 = new ModelRenderer(this, 46, 1);
      this.Shape19.addBox(0.0F, 0.0F, 0.0F, 8, 6, 1);
      this.Shape19.setRotationPoint(0.0F, 0.0F, 8.0F);
      this.Shape19.setTextureSize(64, 32);
      this.Shape19.mirror = true;
      this.setRotation(this.Shape19, 0.0F, 0.0F, 0.0F);
      this.Shape20 = new ModelRenderer(this, 5, 21);
      this.Shape20.addBox(0.0F, 0.0F, 0.0F, 6, 1, 1);
      this.Shape20.setRotationPoint(1.0F, 6.0F, 7.7F);
      this.Shape20.setTextureSize(64, 32);
      this.Shape20.mirror = true;
      this.setRotation(this.Shape20, 0.0F, 0.0F, 0.0F);
      this.Shape21 = new ModelRenderer(this, 5, 17);
      this.Shape21.addBox(0.0F, 0.0F, 0.0F, 1, 1, 2);
      this.Shape21.setRotationPoint(0.0F, 6.0F, 6.0F);
      this.Shape21.setTextureSize(64, 32);
      this.Shape21.mirror = true;
      this.setRotation(this.Shape21, 0.0F, 0.0F, 0.0F);
      this.Shape22 = new ModelRenderer(this, 12, 17);
      this.Shape22.addBox(0.0F, 0.0F, 0.0F, 1, 1, 2);
      this.Shape22.setRotationPoint(7.0F, 6.0F, 6.0F);
      this.Shape22.setTextureSize(64, 32);
      this.Shape22.mirror = true;
      this.setRotation(this.Shape22, 0.0F, 0.0F, 0.0F);
      this.Shape23 = new ModelRenderer(this, 46, 18);
      this.Shape23.addBox(0.0F, 0.0F, 0.0F, 1, 3, 8);
      this.Shape23.setRotationPoint(8.0F, 0.0F, 0.0F);
      this.Shape23.setTextureSize(64, 32);
      this.Shape23.mirror = true;
      this.setRotation(this.Shape23, 0.0F, 0.0F, 0.0F);
      this.Shape24 = new ModelRenderer(this, 20, 20);
      this.Shape24.addBox(0.0F, 0.0F, 0.0F, 1, 2, 1);
      this.Shape24.setRotationPoint(8.0F, 3.0F, 0.0F);
      this.Shape24.setTextureSize(64, 32);
      this.Shape24.mirror = true;
      this.setRotation(this.Shape24, 0.0F, 0.0F, 0.0F);
      this.Shape25 = new ModelRenderer(this, 33, 22);
      this.Shape25.addBox(0.0F, 0.0F, 0.0F, 1, 2, 5);
      this.Shape25.setRotationPoint(8.0F, 3.0F, 3.0F);
      this.Shape25.setTextureSize(64, 32);
      this.Shape25.mirror = true;
      this.setRotation(this.Shape25, 0.0F, 0.0F, 0.0F);
      this.Shape26 = new ModelRenderer(this, 27, 12);
      this.Shape26.addBox(0.0F, 0.0F, 0.0F, 1, 1, 8);
      this.Shape26.setRotationPoint(8.0F, 5.0F, 0.0F);
      this.Shape26.setTextureSize(64, 32);
      this.Shape26.mirror = true;
      this.setRotation(this.Shape26, 0.0F, 0.0F, 0.0F);
      this.Shape27 = new ModelRenderer(this, 5, 13);
      this.Shape27.addBox(0.0F, 0.0F, 0.0F, 1, 1, 2);
      this.Shape27.setRotationPoint(7.7F, 3.0F, 1.0F);
      this.Shape27.setTextureSize(64, 32);
      this.Shape27.mirror = true;
      this.setRotation(this.Shape27, 0.0F, 0.0F, 0.0F);
      this.Shape28 = new ModelRenderer(this, 0, 16);
      this.Shape28.addBox(0.0F, 0.0F, 0.0F, 1, 1, 1);
      this.Shape28.setRotationPoint(7.7F, 4.0F, 2.0F);
      this.Shape28.setTextureSize(64, 32);
      this.Shape28.mirror = true;
      this.setRotation(this.Shape28, 0.0F, 0.0F, 0.0F);
      this.Shape29 = new ModelRenderer(this, 0, 13);
      this.Shape29.addBox(0.0F, 0.0F, 0.0F, 1, 1, 1);
      this.Shape29.setRotationPoint(7.5F, 4.0F, 1.0F);
      this.Shape29.setTextureSize(64, 32);
      this.Shape29.mirror = true;
      this.setRotation(this.Shape29, 0.0F, 0.0F, 0.0F);
      this.Shape30 = new ModelRenderer(this, 1, 1);
      this.Shape30.addBox(0.0F, 0.0F, 0.0F, 8, 1, 8);
      this.Shape30.setRotationPoint(0.0F, -1.0F, 0.0F);
      this.Shape30.setTextureSize(64, 32);
      this.Shape30.mirror = true;
      this.setRotation(this.Shape30, 0.0F, 0.0F, 0.0F);
   }

   public void render(Entity entity, float f, float f1, float f2, float f3, float f4, float f5) {
      super.render(entity, f, f1, f2, f3, f4, f5);
      this.setRotationAngles(f, f1, f2, f3, f4, f5, entity);
      this.Shape2.render(f5);
      this.Shape3.render(f5);
      this.Shape4.render(f5);
      this.Shape5.render(f5);
      this.Shape6.render(f5);
      this.Shape7.render(f5);
      this.Shape8.render(f5);
      this.Shape9.render(f5);
      this.Shape10.render(f5);
      this.Shape11.render(f5);
      this.Shape12.render(f5);
      this.Shape13.render(f5);
      this.Shape14.render(f5);
      this.Shape15.render(f5);
      this.Shape16.render(f5);
      this.Shape17.render(f5);
      this.Shape18.render(f5);
      this.Shape19.render(f5);
      this.Shape20.render(f5);
      this.Shape21.render(f5);
      this.Shape22.render(f5);
      this.Shape23.render(f5);
      this.Shape24.render(f5);
      this.Shape25.render(f5);
      this.Shape26.render(f5);
      this.Shape27.render(f5);
      this.Shape28.render(f5);
      this.Shape29.render(f5);
      this.Shape30.render(f5);
   }

   private void setRotation(ModelRenderer model, float x, float y, float z) {
      model.rotateAngleX = x;
      model.rotateAngleY = y;
      model.rotateAngleZ = z;
   }

   public void setRotationAngles(float f, float f1, float f2, float f3, float f4, float f5, Entity entity) {
      super.setRotationAngles(f, f1, f2, f3, f4, f5, entity);
   }
}
