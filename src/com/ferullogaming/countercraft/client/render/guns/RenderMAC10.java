...
import com.ferullogaming.countercraft.client.model.guns.ModelMAC10;
...
public class RenderMAC10 extends RenderGun {
   public void renderHandLocation(EntityPlayer entityplayer, ItemStack itemstack, boolean par3Right) {
      if (par3Right) {
         ...
         GL11.glTranslatef(-0.07F, 0.15F, -0.1F);
      }

   }

   public void renderOnHUD(ItemStack stack, double x, double y) {
      this.renderGunGUI(stack, x - 14.0D, y - 11.0D, 28.0D, 0.0D, 0.0D, 0.0D);
   }

   public void renderInventorySlot(ItemStack stack, double x, double y) {
      this.renderGunGUI(stack, x + 9.0D, y - 12.0D, 22.0D, -20.0D, 0.0D, 0.0D);
   }
   
   public void renderSkins(ItemStack stack, double x, double y){ this.renderGunGUI2(stack, x + 9.0D, y - 12.0D, 22.0D, -20.0D, 0.0D, 0.0D); }
   ...
   public StickerPosition getSticker0Position() {
      ...
      stickerPos.stickerPosX = -590.0D;
      stickerPos.stickerPosY = 0.0D;
      stickerPos.stickerPosZ = -135.0D;
