...
import com.ferullogaming.countercraft.client.model.hats.ModelHatSkull;
...
public class RenderHatSkull extends RenderHat {
   public RenderHatSkull() {
      super("skullhat");
   }

   public void renderInventorySlot(ItemStack stack, double x, double y) {
      this.renderDisplayed(stack, x - 28.0D, y - 10.0D, 18.0D, false, 0.0D);
   }

   public void renderInspection(ItemStack stack, double x, double y, double rot) {
      this.renderDisplayed(stack, x - 135.0D, y - 20.0D, 100.0D, false, rot);
   }
   ...
   public void renderItemValues(ItemStack par1, ItemRenderType par2) {
      ...
      if (par2 == ItemRenderType.ENTITY || par2 == ItemRenderType.INVENTORY) {
         ...
         GL11.glTranslated(-0.7D, 0.0D, 0.0D);
         scale = 1.5D;
   ...
   public void renderOnHead(ItemStack par1) {
      ...
      GL11.glTranslated(-0.3D, -0.55D, -0.44D);
      GL11.glRotated(0.0D, 0.0D, 1.0D, 0.0D);
      double scale = 1.2D;
      GL11.glScaled(scale, scale + 0.1D, scale + 0.3D);
      super.model.render((Entity)null, 0.0F, 0.0F, 0.0F, 0.0F, 0.0F, 0.0625F);
   }

   public ModelBase getModel() {
      return new ModelHatSkull();
   }
}
