...
import com.ferullogaming.countercraft.client.cloud.packet.PacketInventoryOpenStickerCapsule;
...
public class GuiCCMenuOpenCapsule extends GuiCCMenu implements IGuiFGPromptYesNo {
   private CloudItemStack capsuleOpeningUUID;
   ...
   private boolean isRequestingCapsuleOpen = false;

   public GuiCCMenuOpenCapsule(CloudItemStack par1) {
      this.capsuleOpeningUUID = par1;
   }
   ...
   public void initGui() {
      ...
      int width1 = 1;
      int height1 = 1;
      String var1 = "" + ((CloudItemStickerCapsule)this.capsuleOpeningUUID.getCloudItem()).costToOpen + "c";
      ...
      if (this.isRequestingCapsuleOpen || this.isRolling) {
   ...
   public ArrayList getPotentailStacks() {
      ...
      CloudItemStickerCapsule capsule = (CloudItemStickerCapsule)this.capsuleOpeningUUID.getCloudItem();

      for(int i = 0; i < capsule.contentDisplay.size(); ++i) {
         CloudItemStack stack = new CloudItemStack("fake", ((CaseContent)capsule.contentDisplay.get(i)).getCloudItem().getID());
         list.add(stack);
      }
   ...
   public void onResultsIn(ArrayList par1) {
      ...
      int width1 = 237;
      int height1 = 1;
      if (this.isRequestingCapsuleOpen) {
         ...
         this.isRequestingCapsuleOpen = false;
      }
   ...
   public void actionPerformed(GuiButton par1GuiButton) {
      ...
      if (par1GuiButton.id == 30 && !this.isRequestingCapsuleOpen && !this.isRolling) {
         String var1 = "" + ((CloudItemStickerCapsule)this.capsuleOpeningUUID.getCloudItem()).costToOpen + " Emeralds";
         GuiFGYesNoPrompt gui = (new GuiFGYesNoPrompt(1, this)).addInformation("Opening the capsule will cost:", EnumChatFormatting.GREEN + "" + var1, "Continue?");
   ...
   public void drawScreen(int par1, int par2, float par3) {
      ...
      CCRenderHelper.renderCenteredText(EnumChatFormatting.YELLOW + "" + this.capsuleOpeningUUID.getDisplayName(), x1 + width1 / 2, y1 + 3);
   ...
   public void onResult(int par1, boolean par2) {
      if (par1 == 1 && par2) {
         ClientCloudManager.sendPacket(new PacketInventoryOpenStickerCapsule(this.capsuleOpeningUUID.getUUID()));
         this.openButton.enabled = false;
         this.isRequestingCapsuleOpen = true;
      }
