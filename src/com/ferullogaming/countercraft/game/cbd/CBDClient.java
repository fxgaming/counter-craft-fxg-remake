...
public class CBDClient extends IGameClient {
   private int tempScoreRed = 0;
   private int tempScoreBlue = 0;
   private int tempTime = 0;
   private CompetitiveBombDefusal compGame;
   ...
   public CBDClient(IGame par1) {
      super(par1);
      this.compGame = (CompetitiveBombDefusal)par1;
   }
   ...
   public void onClientUpdate(Minecraft mc) {
      ...
      int i;
      for(i = 0; i < this.compGame.bombPoints.size(); ++i) {
         ((BombPoint)this.compGame.bombPoints.get(i)).onClientUpdate();
      }

      if (this.bombPointParticleDelay++ > 20) {
         for(i = 0; i < this.compGame.bombPoints.size(); ++i) {
            ((BombPoint)this.compGame.bombPoints.get(i)).doParticles();
         }
   ...
   public void onRenderWorld(RenderGlobal par1, float par2) {
      for(int i = 0; i < this.compGame.bombPoints.size(); ++i) {
         ((BombPoint)this.compGame.bombPoints.get(i)).doRender(par2);
      }
   ...
   public void onRenderHUD(Minecraft mc, int par1, int par2) {
      ...
      int mheight = par2 / 2;
      float alpha = 0.6F;
      if (this.compGame.getClientSide().getCurrentVote() != null) {
         Vote vote = this.compGame.getClientSide().getCurrentVote();
      ...
      if (this.compGame.getStatus() == GameStatus.PREGAME && this.compGame.timerPreGame.getTimeRemainingSeconds() < 11) {
         matchTime = EnumChatFormatting.RED + "" + EnumChatFormatting.BOLD + matchTime;
      }

      if (this.compGame.getStatus() == GameStatus.GAME) {
         if (this.compGame.timerPreRound.getTimeRemaining() > 0) {
            matchTime = EnumChatFormatting.RED + "" + EnumChatFormatting.BOLD + matchTime;
         }

         if (this.compGame.timerRound.getTimeRemaining() > 0 && this.compGame.timerRound.getTimeRemainingSeconds() <= 11) {
            matchTime = EnumChatFormatting.RED + "" + EnumChatFormatting.BOLD + matchTime;
         }

         if (this.compGame.isBombPlanted) {
            matchTime = EnumChatFormatting.RED + "" + EnumChatFormatting.BOLD + matchTime;
         }
      }
      ...
      if (this.compGame.getStatus() == GameStatus.POSTGAME && this.compGame.timerPostGame.getTimeRemainingSeconds() < 11) {
         matchTime = EnumChatFormatting.RED + "" + EnumChatFormatting.BOLD + matchTime;
      }
      ...
      boolean y1;
      if (this.compGame.getStatus() == GameStatus.PREGAME) {
         y1 = true;
         CCRenderHelper.renderCenteredTextScaled(EnumChatFormatting.WHITE + "Ожидание игроков " + (this.tempTeamBlue.size() + this.tempTeamRed.size()) + "/10", mwidth, mheight / 2 - 32, 2.0D);
      }

      if (this.compGame.getStatus() == GameStatus.POSTGAME) {
         y1 = true;
   ...
   public void onRenderTAB(Minecraft mc, int par1, int par2) {
      ...
      CCRenderHelper.renderText(EnumChatFormatting.WHITE + "Round: " + EnumChatFormatting.BOLD + this.compGame.currentRound, sbx + 4, sby + 25);
      ...
      CCRenderHelper.renderPlayerScoreboard(sbx, sby + 41, sbwidth, 13, 3, "", EnumChatFormatting.WHITE, this.getGame(), "ПИНГ", "ДЕНЬГИ", "У", "С", "СЧ");
   ...
   public String getNextSpectatingUsername(int par1, String username) {
      ArrayList teamList = super.gameHandler.getPlayerEventHandler().getPlayerTeam(username).getPlayers();
      if (par1 == 0) {
         ++super.spectateIndex;
      } else if (par1 == 1) {
         --super.spectateIndex;
   ...
   public int getCurrentTimeTick() {
      GameStatus status = this.compGame.getStatus();
      if (status == GameStatus.IDLE) {
         return 0;
      } else if (status == GameStatus.PREGAME) {
         return this.compGame.timerPreGame.getTimeRemaining();
      } else {
         if (status == GameStatus.GAME) {
            if (this.compGame.timerPreRound.getTimeRemaining() > 0) {
               return this.compGame.timerPreRound.getTimeRemaining();
            }

            if (this.compGame.timerRound.getTimeRemaining() > 0) {
               return this.compGame.timerRound.getTimeRemaining();
            }

            if (this.compGame.timerPostRound.getTimeRemaining() > 0) {
               return this.compGame.timerPostRound.getTimeRemaining();
            }
         } else if (status == GameStatus.POSTGAME) {
            return this.compGame.timerPostGame.getTimeRemaining();
         }
