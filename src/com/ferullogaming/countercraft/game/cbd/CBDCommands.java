...
public class CBDCommands extends IGameCommandHandler {
   public CBDCommands(IGame par1) {
      super(par1);
   }

   public void onEditCommandCalled(EntityPlayer par1, String[] args) {
      CompetitiveBombDefusal compGame = (CompetitiveBombDefusal)this.getGame();
      CBDPlayerHandler playerHandler = (CBDPlayerHandler)this.getGame().getPlayerEventHandler();
      if (args.length >= 1) {
         String var1 = args[0];
         ...
         if (var1.equals("maxrounds") && args.length >= 2) {
            try {
               var21 = Integer.parseInt(args[1]);
               CommandGameManage.sendChat(par1, "Max Rounds set to [" + var21 + "] minute(s).");
