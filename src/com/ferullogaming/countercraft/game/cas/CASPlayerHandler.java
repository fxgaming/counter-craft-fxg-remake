...
public class CASPlayerHandler extends IGamePlayerHandler {
   ...
   public void onPlayerSwitchTeam(EntityPlayer par1) {
      CasualBombDefusal tdm = (CasualBombDefusal)this.getGame();
      if (tdm.getStatus() == GameStatus.POSTGAME) {
         GameHelper.sendChatToPlayer(tdm, par1.username, "Игра", "Вы не можете поменять команду сейчас.");
      } else {
         ...
         if (newTeam != null && newTeam.getPlayers().size() < team.getPlayers().size()) {
            ...
            GameHelper.sendChatToPlayer(tdm, par1.username, "Игра", "Слишком много игроков на смену команды!");
         }
   ...
   public void onPlayerJoined(EntityPlayer par1, String par2Team) {
      ...
      GameHelper.sendChatToPlayer(this.getGame(), par1.username, "Игра", "/c " + EnumChatFormatting.ITALIC + "Приветствуем " + par1.username + " в обычном режиме.");
      GameHelper.sendChatToPlayer(this.getGame(), par1.username, "Игра", "/c " + team.teamColor + "" + EnumChatFormatting.BOLD + "Назначение в команду " + par2Team + "");
      GameHelper.sendChatToPlayer(this.getGame(), par1.username, "Игра", "");
      GameHelper.sendChatToPlayer(this.getGame(), par1.username, "Игра", "" + EnumChatFormatting.WHITE + ChatSymbols.SQUARE(30));
      ...
      GameHelper.sendGameNotification(par1.username, team.teamColor + "" + team.teamName + " Team");
      GameHelper.setPlayerGameValue(this.getGame(), par1.username, "economy", ((CasualBombDefusal)this.getGame()).getEconomyStartingAmount());
      ...
      CasualBombDefusal cas = (CasualBombDefusal)this.getGame();
      if (cas.getStatus() == GameStatus.GAME && !cas.isRoundEnded) {
   ...
   public void onPlayerExit(EntityPlayer par1) {
      if (par1 != null) {
         ...
         CasualBombDefusal comp = (CasualBombDefusal)this.getGame();
         ...
         par1.inventory.clearInventory(-1, -1);
         this.temporaryData.put(par1.username, Arrays.asList(String.valueOf(comp.getPlayerGameData(par1.username).getInteger("kills")), String.valueOf(comp.getPlayerGameData(par1.username).getInteger("deaths")), String.valueOf(comp.getPlayerGameData(par1.username).getInteger("score"))));
      }
   ...
   public boolean allowItemTossed(EntityPlayer par1, ItemStack par2) {
      if (par2.getItem() instanceof ItemGun) {
         ...
         GameHelper.sendChatToTeam(this.getGame(), this.getTeam("red"), "Игра", par1.username + " выкинул бомбу!");
         return true;
      } else {
   ...
   public boolean allowItemPickup(EntityPlayer par1, ItemStack par2, EntityItem par3) {
      ...
      if (data.isGhost) {
         ...
         if (par2.getItem() instanceof ItemGun) {
            ...
            GameHelper.setPlayerGameValue(this.getGame(), par1.username, "itempickedup", 1);
            GameHelper.sendChatToTeam(this.getGame(), this.getTeam("red"), "Игра", par1.username + " подобрал бомбу!");
            return true;
         }
      ...
      if (((IGameComponentEconomy)this.getGame()).hasPlayerEconomy(par1.username, var1)) {
         ...
         if (par2 != null && par2.getItem() != null && par2.getItem() instanceof ItemArmorCC) {
            if (!this.getGame().getStatus().equals(GameStatus.PREGAME)) {
               ...
               GameHelper.sendChatToPlayer(this.getGame(), par1.username, "Игра", "Вы не можете купить броню на разминке!");
            }
         } else {
   ...
   public boolean acceptsPlayersFromMM() {
      return this.getGame().getStatus() != GameStatus.POSTGAME;
   }
