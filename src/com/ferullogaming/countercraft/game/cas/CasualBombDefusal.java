...
public class CasualBombDefusal extends IGame implements IGameComponentBomb, IGameComponentEconomy {
   public CASPlayerHandler playerHandler = new CASPlayerHandler(this);
   ...
   public int maxRoundTime = 2;
   public int maxRoundWins = 15;
   public int currentRound = 0;
   public String roundWinners = "";
   public String roundMVP = "";
   public boolean isBombPlanted = false;
   public int bombPlantedTime = 900;
   public int economyStartingAmount = 800;
   public boolean isMatchEnded = false;
   public boolean isRoundEnded = false;
   private CASCommands commandHandler = new CASCommands(this);
   private ArrayList leftUsers = new ArrayList();
   private CASClient clientSide = new CASClient(this);
   private boolean isRound = false;

   public CasualBombDefusal() {
      super("cas");
      this.restartGame();
   }
   ...
   public void onUpdate(World par1) {
      ...
      if (this.getStatus() != GameStatus.IDLE && this.getPlayerEventHandler().getPlayers().size() == 0) {
         ...
         if (this.getPlayerEventHandler().getPlayers().size() >= 4) {
            this.timerPreGame.updateTimer();
         } else {
            this.timerPreGame.setTimeRemainingSeconds(45);
         }
         ...
         if (!this.isMatchEnded) {
            ...
            if (teamBlueScore >= this.maxRoundWins / 2) {
               this.endMatch(teamBlue, true);
               return;
            }

            if (teamRedScore >= this.maxRoundWins / 2) {
               this.endMatch(teamRed, true);
               return;
            }

            if (teamBlueScore == teamRedScore && teamBlueScore == this.maxRoundWins / 2 - 1) {
   ...
   public void endMatch(Team par1, boolean par2) {
      if (!this.isMatchEnded) {
         ...
         GameHelper.refreshPlayersMCLevels(this.playerHandler.getPlayers());
         ((CASPlayerHandler)this.getPlayerEventHandler()).onRoundRefreshed(this.playerHandler.getPlayers());
         this.playerHandler.getTeam("Red").tpTeamSetSpawns();
         this.playerHandler.getTeam("Blue").tpTeamSetSpawns();
         ...
         GameHelper.sendChatToAll(this, "Игра", "/c " + EnumChatFormatting.WHITE + EnumChatFormatting.BOLD + "Обычный режим");
         if (par1 == null) {
         ...
         if (FMLCommonHandler.instance().getSide().isServer()) {
            ...
            if (ServerManager.instance().isMatchMakingAccepted && par1 != null) {
   ...
   public void setupRound(int par1) {
      ...
      this.timerPreRound = new Timer(0, 8);
   ...
   public void onBombPlanted(World par1, EntityPlayer par2) {
      ...
      GameHelper.sendGameNotification(this.getPlayerEventHandler().getPlayers(), EnumChatFormatting.RED + "Бомба заложена!");
   ...
   public void onBombDefused(World par1, String par2) {
      ...
      GameHelper.sendGameNotification(this.getPlayerEventHandler().getPlayers(), EnumChatFormatting.BLUE + "Бомба обезврежена!");
   ...
   public int getEconomyStartingAmount() {
      return this.economyStartingAmount;
   }
}
